...
public class Group implements Parcelable {
    ...
    public void downloadAvatar(DownloadManager downloadManager, String quality) {
        if(quality.equals("medium")) {
            downloadManager.downloadOnePhotoToCache(avatar_msize_url, String.format("avatar_%s", id), "group_avatars");
        } else if(quality.equals("high")) {
            if(avatar_hsize_url.length() == 0) {
                avatar_hsize_url = avatar_msize_url;
            }
            downloadManager.downloadOnePhotoToCache(avatar_hsize_url, String.format("avatar_%s", id), "group_avatars");
        } else if(quality.equals("original")) {
            if(avatar_osize_url.length() == 0) {
                avatar_osize_url = avatar_msize_url;
            }
            downloadManager.downloadOnePhotoToCache(avatar_osize_url, String.format("avatar_%s", id), "group_avatars");
        }
    }
