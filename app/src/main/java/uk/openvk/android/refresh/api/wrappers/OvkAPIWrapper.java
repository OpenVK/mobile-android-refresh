...
public class OvkAPIWrapper {
    ...
    public OvkAPIWrapper(Context ctx, boolean use_https, Handler handler) {
            ...
            public void run() throws OutOfMemoryError {
                try {
                    ...
                    try {
                        ...
                        if (Objects.requireNonNull(ex.getMessage()).startsWith("Authorization required")) {
                            response_code = 401;
                            sendMessage(HandlerMessages.TWOFACTOR_CODE_REQUIRED, response_body);
            ...
            public void run() {
                try {
                    ...
                    try {
                        ...
                        if(Objects.equals(e.getMessage(), "Scheme 'https' not registered.")) {
        ...
        Runnable httpRunnable = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    if(Objects.requireNonNull(ex.getMessage()).startsWith("Expected status code 2xx")) {
                        String code_str = ex.getMessage().substring
                                (ex.getMessage().length() - 3);
                        response_code = Integer.parseInt(code_str);
